...
using FalconeOne.BLL.HubsContracts;
...
namespace FalconeOne.BLL.Services
{
    public class UserService : BaseService, IUserService
    {
        ...
        public UserService(UserManager<User> userManager,
            ...
            IConfiguration configuration, IIdenticonProvider identiconProvider, IHubContext<UserDashboardHub, IUserDashboardHub> userDashboardHubContext, IUserPredictionService userPredictionService) : base(userManager, unitOfWork, httpContextAccessor, configuration, tenantService, identiconProvider)
        {
        ...
        {
            var user = await _unitOfWork.UserRepository.FindAsync(x => x.ResourceAlias == model.ResourceId, cancellationToken);
            var permission = await _unitOfWork.PermissionRepository.FindAsync(x => x.Type == ClaimTypes.System && x.Value == model.Permission, cancellationToken);
        ...
        public async Task<UserDto> GetUserByResourceId(string resourceId, CancellationToken cancellationToken)
        {
            ...
            var attempts = await _userManager.Users.Where(x => x.Id == result.Id).SelectMany(x => x.LoginAttempts).ToListAsync();

            if(attempts is not null && attempts.Any())
            {
                var data = TransformLoginAttempts(attempts);

                var prediction = _userPredictionService.PredictNextLoginProbability(data);

                user.LockoutProbability = $"{prediction.Probability * 100:F0}%";
            }
            return user;
        }
        ...
        {
            var user = await _unitOfWork.UserRepository.FindAsync(x => x.ResourceAlias == resourceId, cancellationToken);
        ...
        {
            var user = await _unitOfWork.UserRepository.FindAsync(expression: x => x.Id == userId,
        ...
        public async Task<bool> UpsertTenantUser(UpsertTenantUserDto model, CancellationToken cancellationToken)
        {
            ...
            {
                var tenant = await _unitOfWork.TenantRepository.FindAsync(x => x.AccountAlias == model.AccountAlias, cancellationToken);
        ...
        public async Task<bool> AddUsersToSecurityGroup(AddUsersToSecurityGroupDto model, CancellationToken cancellationToken)
        {
            ...
            {
                var isUserPresent = await _unitOfWork.TenantUserSecurityGroupRepository.FindAsync(x => x.SecurityGroupId == group.Id && x.TenantUser.UserId == user, cancellationToken);
