...
namespace FalconeOne.BLL.Services
{
    public class BaseService
    {
        ...
        protected List<UserLoginAttemptData> TransformLoginAttempts(List<UserLoginAttempt> loginAttempts)
        {
            var transformedData = new List<UserLoginAttemptData>();

            var orderedAttempts = loginAttempts.OrderBy(attempt => attempt.LoginAttemptDate).ToList();

            for (int i = 0; i < orderedAttempts.Count; i++)
            {
                var currentAttempt = orderedAttempts[i];
                var previousAttempt = i > 0 ? orderedAttempts[i - 1] : null;

                float timeSinceLastLogin = previousAttempt != null
                    ? (float)(currentAttempt.LoginAttemptDate - previousAttempt.LoginAttemptDate).TotalDays
                    : 0;

                float numberOfFailedAttempts = orderedAttempts
                    .Take(i)
                    .Count(a => !a.IsSucessfull);

                float dayOfWeek = (float)currentAttempt.LoginAttemptDate.DayOfWeek;

                transformedData.Add(new UserLoginAttemptData
                {
                    TimeSinceLastLogin = timeSinceLastLogin,
                    NumberOfFailedAttempts = numberOfFailedAttempts,
                    DayOfWeek = dayOfWeek,
                    IsSucessfull = currentAttempt.IsSucessfull
                });
            }
